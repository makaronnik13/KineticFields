...
public class BeatDetector : MonoBehaviour
{
	...
	private int highsCount = 5000;
	private Queue<float> highs = new Queue<float>();
	private int MIN_BPM = 20;
	private int MAX_BPM = 300;
	private const int BASE_FREQUENCY = 44100;
	// Base channels (2ch)
	private const int BASE_CHANNELS = 1;
	// Base split size of sample data (case of 44.1kbps & 2ch)
	private const int BASE_SPLIT_SAMPLE_SIZE = 4410;
	
	private BpmMatchData[] bpmMatchDatas;
	private int SearchBpm(float[] volumeArr, int frequency, int splitFrameSize)
	{
		bpmMatchDatas = new BpmMatchData[MAX_BPM - MIN_BPM + 1];
	...
	void Analyse ()
	{
		...
		if (highs.Count>highsCount)
		{
			highs.Dequeue();
		}
		Debug.Log(SearchBpm(highs.ToArray(), BASE_FREQUENCY, splitFrameSize));
